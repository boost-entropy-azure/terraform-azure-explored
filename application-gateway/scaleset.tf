resource "azurerm_virtual_machine_scale_set" "demo" {
  ...
  extension {
    ...
    settings             = <<SETTINGS
        {
          "fileUris": ["https://raw.githubusercontent.com/narenarjun/terraform-azure-explored/master/application-gateway/nginx-install.sh"],
          "commandToExecute": "./nginx-install.sh"
        }
      SETTINGS
  }
